...
- `actions.log` — an append-only record of memcap's enforcement decisions, not
  ...
  or didn't do something, this is where to look. Kill records are logged every
  time; the two lines that would otherwise repeat on every single pass — tier 3
  declining, and the combined cap being exceeded — are throttled to at most one
  per `LOG_THROTTLE_SEC` so they don't drown the kill records in a long-running
  install.
