...
So the bash 3.2 guarantee rests on three things, and the parse loop is only one of them:
the Global Constraints listing the banned constructs, reviewers checking for them, and
`bats tests/` actually executing the code paths. The parse loop catches genuine syntax
errors, which is worth having, but it is not the whole guarantee and should not be
