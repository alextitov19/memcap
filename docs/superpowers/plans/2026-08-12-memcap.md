...
- Produces: `mc_total_ram_gb` → integer; `mc_cpu_count` → integer; `mc_installed_agents` → space-separated names; `mc_ps_snapshot` → `pid ppid footprint_kb command` lines; `mc_free_pct` → integer
...
# Refuse anything broad enough to sweep unrelated work: must be at least two levels
# below $HOME. This gates a destructive operation -- whatever it accepts becomes a
...
mc_canonicalize() {
  ...
  # Resolve against the nearest EXISTING ancestor, then re-append the remainder.
  ...
  # designating /tmp/proj. The loop is bounded by the component count.
  head="$dir"; tail=""
...
# $HOME, judged on the canonical path. This gates a destructive operation -- whatever
# it accepts becomes a directory where memcap later kills processes.
mc_root_is_safe() {
  local real
...
**Interfaces:**
- Consumes: `mc_classify`, `mc_ps_snapshot`, `mc_free_pct`, `mc_cap_gb`, `mc_docker_gb`, `mc_total_ram_gb`
- Produces: `mc_render_status` → human-readable report on stdout

Note: no Docker-specific memory correction exists or is needed. `mc_ps_snapshot` already
reports physical footprint for every process, which is what the old correction existed to
recover for the VM alone.
...
case "${1:-help}" in
  ...
  clean)  . "$LIB/enforce.sh"
          ...
          eval "$(mc_ps_snapshot | mc_classify)"; mc_reap_orphans; mc_reap_sims ;;
...
# Must have a default: a bare $MC_DRY_RUN aborts under `set -u` on a plain
# `memcap docker apply`, and that abort would fire BEFORE the containers-running
# deferral check, defeating this module's core safety guarantee.
MC_DRY_RUN="${MC_DRY_RUN:-0}"
...
# The `|| [[ ... ]]` form this replaced passed vacuously: setup()'s config had no
# TIER2_MIN_AGE_SEC line at all, so the grep always failed and the first branch was
# always true. It asserted nothing. Seed the key, then assert the exact value survives.
...
- [ ] **Step 0: Make the repo satisfy the constraint CI is about to enforce**

`tests/scaffold.bats` (from Task 1) sources `libexec/common.sh` dynamically with no
`# shellcheck source=/dev/null` directive, so `shellcheck tests/scaffold.bats` emits
SC1091 when the file is checked on its own. Every other bats file already carries the
directive. Add it there too:
...
Verify with `shellcheck tests/scaffold.bats` alone — checking it alongside the libexec
files masks the warning, which is why this went unnoticed until Task 4.
    ...
    steps:
      ...
      - name: Shellcheck
        run: |
          ...
          # `|| exit 1` rather than relying on the runner injecting `-e`: a bare loop's
          # exit status is its LAST iteration, so without this the step passes whenever
          # the final file happens to be clean. Making it self-failing removes the
          # dependency on an ambient shell option a future `shell:` key could drop.
          # tests/*.bash too: helper.bash is executed by all ten bats files via
          # `load helper` and sat in neither loop. Checked individually, not grouped
          # with the .bats files -- grouping reopens the source-resolution masking.
          for f in tests/*.bats tests/*.bash; do
            shellcheck "$f" || exit 1
          done
...
Run (wrapped in `bash -c` so a failure exits the subshell rather than your terminal):

```bash
bash -c '
  shellcheck bin/memcap libexec/*.sh || exit 1
  for f in tests/*.bats; do shellcheck "$f" || exit 1; done
  for f in bin/memcap libexec/*.sh; do /bin/bash -n "$f" || exit 1; done
  bats tests/
'
```
Expected: all clean.
...
gh repo create alextitov19/memcap --private --source=. --push
```
...
```bash
gh repo create alextitov19/homebrew-memcap --private --clone
...
Both repos are created **private** by the author's decision; flipping them public is a
deliberate act they take themselves, not something this plan performs.

That changes verification. Homebrew can clone a private tap you have access to, but
downloading a release tarball from a private repo needs a token, so
`brew install alextitov19/memcap/memcap` will not work end-to-end until the repo is
public. Verify the formula mechanics against a local tarball instead — that exercises
everything except the download URL:

```bash
# build the same tarball the release would serve
git archive --format=tar.gz --prefix=memcap-0.1.0/ -o /tmp/memcap-0.1.0.tar.gz v0.1.0
shasum -a 256 /tmp/memcap-0.1.0.tar.gz

# point the formula at it temporarily
#   url "file:///tmp/memcap-0.1.0.tar.gz"
#   sha256 "<from above>"

brew install --build-from-source ./Formula/memcap.rb
memcap help
memcap init
brew services start memcap
brew services list | grep memcap   # expect: started
memcap status                      # expect: a real report
```

Then restore the formula's `url` to the GitHub release URL and the release tarball's
own sha256, so the committed formula is the one that will work once the repo is public.
Record in the report that the download URL itself is the single unverified element.
...
**Files:** none in the repo. This task changes the author's live system.

Run memcap and the prototype **in parallel first**, and retire the prototype only once
the two are shown to agree on real work. memcap has 104 tests and extensive manual
verification behind it, but it has never made an unattended decision on this machine.
Both systems are idempotent, both refuse to kill an agent CLI, and both age-gate their
kills — so an overlap is safe. Worst case each reaps orphans the other would have.

#### Phase A — start the parallel run

- [ ] **Step A1: Install from the tap and start the service**

```bash
brew tap alextitov19/memcap            # private tap; you have access
brew install alextitov19/memcap/memcap # once the repo is public; until then:
                                       #   brew install --build-from-source <local formula>
memcap init                            # accept the defaults; decline the Docker ceiling
brew services start memcap
brew services list | grep memcap       # expect: started
```

- [ ] **Step A2: Confirm both are running and neither is confused by the other**

```bash
launchctl print gui/$(id -u)/com.alextitov.agent-ram-guard | rg 'runs ='   # prototype
brew services list | rg memcap                                            # memcap
memcap status
~/.claude/hooks/agent-ram-guard.sh --report
```

The two reports should now agree closely — the prototype was backported to memcap's
footprint measurement on 2026-08-13. A large divergence means something is wrong and is
worth investigating before going further.

- [ ] **Step A3: Leave both running for at least a day of normal work.**

#### Phase B — compare, then retire

- [ ] **Step B1: Compare what each actually did**

```bash
tail -50 ~/.local/state/memcap/actions.log      # memcap
tail -50 ~/.claude/agent-ram-guard.log          # prototype
tail -20 ~/.claude/session-cleanup.log
```

Confirm: they identify the same orphans; neither killed an agent CLI; neither killed a
process the other considered protected; memcap's tier-2 age gate fired at least once
under real load, or the budget was never exceeded. Investigate any disagreement before
retiring anything.

- [ ] **Step B2: Retire the prototype, only if Step B1 is clean**
...
- [ ] **Step B3: Remove the prototype's SessionEnd hooks**
...
- [ ] **Step B4: Remove the `.zshrc` block**

Delete the `--- agent memory hygiene ---` block and the four `agent-*` aliases, keeping
a `.bak` first. Note `agent-ram`/`agent-cleanup`/`agent-profile` are replaced by
`memcap status`/`memcap clean`/`memcap profile`.

- [ ] **Step B5: Verify memcap alone is enforcing**

```bash
memcap status
tail -5 ~/.local/state/memcap/actions.log
launchctl print gui/$(id -u)/com.alextitov.agent-ram-guard 2>&1 | tail -1  # expect: not found
```

#### Rollback

Every prototype artifact has a backup: `agent-ram-guard.sh.pre-etime.bak`,
`agent-ram-guard.sh.pre-footprint.bak`, `settings.json.pre-memcap.bak` (×3), and the
`.zshrc` backups. Reverting is restoring those and re-running `launchctl bootstrap`.

### Known limitation: static checks do not catch bash-4 *runtime* constructs

`bash -n` is a parser check. A bash-4-only construct that is syntactically valid to
bash 3.2 — `declare -A`, the lowercase-expansion form, `mapfile` — parses fine and fails
only when the line executes. Verified: `declare -A x` planted in a checked file passes
both the shellcheck step and the parse-check step.

So the bash 3.2 guarantee rests on the Global Constraints listing the banned constructs
and on reviewers checking for them. The parse loop catches genuine syntax errors, which
is worth having, but it is not the whole guarantee and should not be described as though
it were.

`bats tests/` is **not** a third layer, despite the temptation to count it. `bats` is
`#!/usr/bin/env bash`, so it runs under whatever bash is first on `PATH` — which on a
GitHub runner is a newer Homebrew bash, the exact reason the parse step pins
`/bin/bash`. A `declare -A` in `libexec/` would therefore execute happily in CI's Tests
step and surface only on an end-user Mac where `/bin/bash` is the only bash available.
Nothing in this workflow catches that class; only discipline and review do.
