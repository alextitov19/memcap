...
- [ ] **Step 1: Confirm memcap reproduces the prototype's BUDGET, and explain the usage gap**

Run: `memcap status` and compare against `~/.claude/hooks/agent-ram-guard.sh --report`.

Expected to match: cap 16 GB, Docker ceiling 6 GB, agent budget 10 GB.

Expected to DIFFER, and this is correct: the usage figures. The prototype sums `ps` RSS
and over-counts shared memory; memcap uses physical footprint. Measured on the author's
machine with a simulator booted, the prototype reported agents 14.48 GB / sims 12.96 GB /
combined 20.90 GB, against memcap's 7.62 / 5.29 / 14.03 — the prototype sat permanently
above its own budget and would fire tier 2 against any dev server older than the age gate.
Docker agrees between them (6.41 vs 6.42) only because the prototype special-cases the VM.

Do not "fix" memcap to match the prototype. Confirm instead that memcap's combined figure
is physically plausible against `vm_stat` active+wired plus compressed.
